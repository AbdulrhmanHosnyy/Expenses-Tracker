...
namespace ExpensesTracker.GUI.CustomerGUI
{
    public partial class CustomerUserControl : UserControl
    {
        //  Variables
        private readonly IDataHelper<Customer> dataHelper;
        ...
        public CustomerUserControl()
        {
            InitializeComponent();
            setPermissions();
            dataHelper = (IDataHelper<Customer>)ConfigrationObjectManager.Get("Customer");
        ...
        {
            AddAndEditCustomerForm addAndEditCustomerForm = new AddAndEditCustomerForm(0, this);
                        ...
                        for (int i = 0; i < IdList.Count; i++)
                        {
                            ...
                            if (result == 1)
                            {
                                ...
                                SystemRecord systemRecord = new SystemRecord
                                {
                                    ...
                                    Details = "Customer with id " + RowId.ToString() + "is deleted",
        ...
        {
            await Task.Run(() => UpdateData(CustomersId));
        }
        ...
        public async void LoadData()
        {
            loadingForm.Show();
            var data = await dataHelper.GetAllDataAsync();
            CustomersId = data.Select(x => x.Id).ToList();
            await Task.Run(() => UpdateData(CustomersId));
        ...
        private void Edit()
        {
            if (dataGridView1.RowCount > 0)
            {
                ...
                AddAndEditCustomerForm addAndEditCustomerForm =
                    new AddAndEditCustomerForm(RowId, this);
                addAndEditCustomerForm.Show();
            }
            else
            {
        ...
        private void UpdateData(List<int> CustomersId)
        {
            //  Get Data
            //  Loop Into CustomersId
            for (int i = 0; i < CustomersId.Count; i++)
            {
                var CustomerId = CustomersId[i];
                try
                {
                    IncomeAmount = IncomeDataHelper.GetAllData().Where(x => x.SupplierId == CustomerId)
                ...
                //  Set Data
                Customer customer = dataHelper.GetAllData().Where(x => x.Id == CustomerId).First();
                customer.Balance = IncomeAmount;
                dataHelper.Edit(customer);
            }
        }
