...
namespace ExpensesTracker.GUI.CategoryGUI
{
    public partial class CategoryUserControl : UserControl
    {
        ...
        private static CategoryUserControl categoryUserControl;
        ...
        {
            AddAndEditCategoryForm addAndEditCategoryForm = new AddAndEditCategoryForm(0, this);
        ...
        {
            await Task.Run(() => UpdateData(CategoriesId));
        }
        ...
        {
            return categoryUserControl ?? (new CategoryUserControl());
        }
        ...
        private void SetColumnsTitles()
        {
            ...
            dataGridView1.Columns[5].HeaderText = "AddedDate";
        }
        ...
        private void Edit()
        {
            if (dataGridView1.RowCount > 0)
            {
                ...
                AddAndEditCategoryForm addAndEditCategoryForm =
                    new AddAndEditCategoryForm(RowId, this);
                addAndEditCategoryForm.Show();
            }
            else
            {
        ...
        private void UpdateData(List<int> CategoriesId)
        {
            ...
            for (int i = 0; i < CategoriesId.Count; i++)
            {
                ...
                try
                {
                    IncomeAmount = IncomeDataHelper.GetAllData().Where(x => x.CategoryId == CategoryId)
                                .Select(x => x.Amount).ToArray().Sum();
                    OutcomeAmount = OutcomeDataHelper.GetAllData().Where(x => x.CategoryId == CategoryId)
                                .Select(x => x.Amount).ToArray().Sum();
                    //  Set Data
                    Category category = dataHelper.GetAllData().Where(x => x.Id == CategoryId).First();
                    category.Balance = IncomeAmount - OutcomeAmount;
                    dataHelper.Edit(category);
                }
                catch
                {
                }
